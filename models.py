...
class VideoModel(nn.Module):
    ...
    def _prepare_DA(self, num_class, base_model):  # convert the model to DA framework
        ...
        if self.ens_DA == 'MCD':
            self.fc_classifier_video_source_2 = nn.Linear(self.output_size,
    ...
    def cdan_frame(self, feat, pred, domain='source'):
        ...
        dc_labels = torch.from_numpy(np.array([[1]] * self.batch_source * self.train_segments + [
            [0]] * self.batch_target * self.val_segments)).float().cuda()
        # if domain == 'source':
        ...
        G_loss = torch.sum(
            weight.view(-1, 1) * nn.BCELoss(reduction='none')(self.netD(features), dc_labels)) / torch.sum(
            weight).detach().item()
        return G_loss
    ...
    def cdan_video(self, feat, pred, domain='source'):
        ...
        G_loss = torch.sum(
            weight.view(-1, 1) * nn.BCELoss(reduction='none')(self.netD2(features), dc_labels)) / torch.sum(
            weight).detach().item()
        return G_loss
    ...
    def forward(self, input_source, source_label, input_target, beta = [0,0,0], mu = 0, is_train = True, reverse= False, batchsize=0, dummy=False):
        ...
        if args.use_attention:
            ...
            print('shape1',input_source.shape)
            ...
            input_source = input_source.transpose(0, 1).view(batch_source, -1, 49, input_source.shape[-1]).transpose(2,
                                                                                                                     3)
            ...
            input_target = input_target.transpose(0, 1).view(batch_source, -1, 49, input_target.shape[-1]).transpose(2,
                                                                                                                     3)
        ...
        print('shape2',input_source.shape)
        ###########=============== end ========
        ...
        feat_base_source0 = input_source.view(-1, input_source.size()[
            -1])  # e.g. 256 x 25 x self.d_model --> 6400 x self.d_model
        feat_base_target0 = input_target.view(-1, input_target.size()[
            -1])  # e.g. 256 x 25 x self.d_model --> 6400 x self.d_model
        if flag:
            feat_base_source = self.feature_gmm_source.view(-1, self.feature_gmm_source.size()[
                -1])  # e.g. 256 x 25 x self.d_model --> 6400 x self.d_model
            feat_base_target = self.feature_gmm_target.view(-1, self.feature_gmm_target.size()[
                -1])  # e.g. 256 x 25 x self.d_model --> 6400 x self.d_model
        else:
            feat_base_source = input_source.view(-1, input_source.size()[
                -1])  # e.g. 256 x 25 x self.d_model --> 6400 x self.d_model
            feat_base_target = input_target.view(-1, input_target.size()[
                -1])  # e.g. 256 x 25 x self.d_model --> 6400 x self.d_model
        ...
        feat_fc_target = self.fc_feature_shared_target(
            feat_base_target) if args.share_mapping == False else self.fc_feature_shared_source(feat_base_target)
        ...
        # class pred - video
        pred_lstm_source = self.lstm_vid_classifier(
            feat_fc_source.view(batch_source, self.train_segments, -1).transpose(0, 1))
        pred_lstm_target = self.lstm_vid_classifier(
            feat_fc_target.view(batch_target, self.val_segments, -1).transpose(0, 1))
        ...
        if args.use_cdan:
            ...
            cdan_loss_v = self.cdan_video(torch.cat(
                [self.domain_classifier_video_pre(feat_fc_source), self.domain_classifier_video_pre(feat_fc_target)]),
                                          torch.cat([pred_lstm_source, pred_lstm_target]))
            # fill the blank
        ...
        if args.use_cdan:
            ...
            labels = torch.cat(
                [pred_fc_source.expand(self.train_segments, batch_source, self.num_class).transpose(0, 1).contiguous(),
                 pred_fc_target.expand(self.val_segments, batch_target, self.num_class).transpose(0, 1).contiguous()])
        ...
        if self.if_trm:
            ...
            feat_fc_video_relation_source = torch.randn(feat_fc_video_source.shape[0],
                                                        feat_fc_video_source.shape[1] - 1, self.num_bottleneck).cuda()
            feat_fc_video_relation_target = torch.randn(feat_fc_video_target.shape[0],
                                                        feat_fc_video_target.shape[1] - 1, self.num_bottleneck).cuda()
            # adversarial branch
            ...
            # feat_fc_video_relation_source = self.TRN(feat_fc_video_source) # 128x5x512 --> 128x5x256 (256-dim. relation feature vectors x 5)
            # feat_fc_video_relation_target = self.TRN(feat_fc_video_target)
            feat_fc_video_source = feat_fc_video_source.view(feat_fc_video_source.shape[0],
                                                             -1)  # reshape based on the segments (e.g. 640x512 --> 128x5x512)
            feat_fc_video_target = feat_fc_video_target.view(feat_fc_video_target.shape[0],
                                                             -1)  # reshape based on the segments (e.g. 640x512 --> 128x5x512)
        ...
        output_source = self.final_output(pred_fc_source, pred_lstm_source, num_segments,
                                          batch_source)  # select output from frame or video prediction

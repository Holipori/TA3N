...
def run(dataset_name = args.source, split = 'train', mode='flow', root='/ssd2/charades/Charades_v1_rgb', batch_size=1, load_model='', save_dir=''):
    # setup dataset
    ...
    if dataset_name == 'hmdb51':
        ...
        dir = '/home/xinyue/dataset/ucf101/flow/'
    sets = os.listdir(dir)
    flag = 0
    for item in sets:
        name = [item[:-4]]
        ...
        # if flag == 0:
        #     if name[0] == 'v_BoxingPunchingBag_g25_c03':
        #         flag = 1
        #     else:
        #         continue
        ...
        print(path)
        inputs = np.load(path)
        inputs = torch.from_numpy(inputs).permute(0, 4, 1, 2, 3).float()
    ...
    #     inputs, labels, name = data
        ...
        feature_dir = save_dir + dataset_name + '/RGB-feature-flow/'
        if t > 1600: # PASS
            features = []
...
if __name__ == '__main__':
    ...
    run(mode='flow', save_dir=args.save_dir, dataset_name = 'ucf101')
